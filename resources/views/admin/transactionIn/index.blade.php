...
@section('container')
    ...
    <div id='recipients' class="p-8 mt-6 lg:mt-0 rounded shadow bg-white border-b-2 border-indigo-600">
        <div class="flex gap-1 mb-10">
            <a href="{{ route('transactionIn.create') }}"
        ...
        <table id="dataItem" class="stripe hover" style="width:100%; padding-top: 1em;  padding-bottom: 1em;">
            <thead>
                <tr>
                    <th data-priority="1">Nama Item</th>
                    <th data-priority="2">Supplier</th>
                    <th data-priority="3">Kuantitas</th>
                    <th data-priority="4">Tanggal Masuk</th>
                    <th>Action</th>
                </tr>
            </thead>
            <tbody>
                @forelse ($transactionIn as $data)
                    <tr>
                        ...
                        <td>
                            <div>
                                ...
                                <a href="{{ route('transactionIn.edit', $data->id) }}"
                                ...
                                <form action="{{ route('transactionIn.destroy', $data->id) }}" method="POST"
    ...
    @include('admin.transactionIn.feature.modal')
@endsection
@section('scripts')
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            viewButtons.forEach(button => {
                button.addEventListener('click', function() {
                    ...
                    fetch(`/transactionIn/${itemId}/detail`)
                        .then(response => response.json())
                        .then(data => {
                            // Update konten modal
                            document.getElementById('modal-name').textContent = data.item.name;
                            document.getElementById('judul-modal').textContent = data.item.name;
                            document.getElementById('modal-supplier').textContent = data
                                .supplier.name;
                            document.getElementById('modal-quantity').textContent = data
                                .quantity;
                            document.getElementById('modal-date').textContent = data
                                .date;
                            document.getElementById('modal-note').textContent = data
                                .note;
                        })
                        .catch(error => {
