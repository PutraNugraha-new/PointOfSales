...
@section('container')
    ...
    <div id='recipients' class="p-8 mt-6 lg:mt-0 rounded shadow bg-white border-b-2 border-indigo-600">
        ...
        <table id="dataCategories" class="stripe hover" style="width:100%; padding-top: 1em;  padding-bottom: 1em;">
            <thead>
                <tr>
            ...
            <tbody>
                @forelse ($categories as $data)
                    <tr>
                        ...
                        <td>{{ $data->updated_at->translatedFormat('F d, Y') }}</td>
                        <td>
                            <div>
                                {{-- <a href="#"
                                    class="inline-flex items-center px-1 py-2 text-md font-medium text-slate-600 hover:underline transition-colors duration-200 ease-in-out">
    ...
    @include('admin.category.feature.modal')
@endsection
@section('scripts')
    <script>
        $(document).ready(function() {
            // Hancurkan instance DataTable jika sudah ada
            if ($.fn.DataTable.isDataTable('#dataCategories')) {
                $('#dataCategories').DataTable().destroy();
            }
